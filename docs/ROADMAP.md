...
已完成里程碑：M0–M4 核心管線、A3 幾何基元擴充（含互動弧形卡尺）、A5 幾何構建（交線/對稱中線/點到線投影）、GD&T v1（真圓度/真直度/平行/垂直/同心，單基準單邊）、N1 配方驗證、Deep-audit P0–P3 修正、UI overlay 殘留修正、**量測編排（RecipeRunner/MeasurementWorkflow）搬進 HALCON-free Application 層並補齊 15 工具派發測試涵蓋**（結構性驗證洞已補；測試 49 套）、**Phase 3 量測穩健性無硬體軟體項**（B1 fuzzy 邊緣、B2 重複性/GR&R、真直度真值）。
...
| 3 | Production Robustness | fuzzy/robust 邊緣量測（B1）+ GR&R/重複性自測（B2） | ✅ **無硬體軟體項完成 2026-07-23**（merge B2/真直度 `9fafb20`、B1 `96d3dac`）：**B1** `HalconEdgeDetector` fuzzy 路徑（`set_fuzzy_measure "contrast"` + `fuzzy_measure_pos`，④診斷 Fuzzy 勾選+門檻，GUI 驗過）；**B2** `Domain/Repeatability/RepeatabilityReport`（6σ/GR&R%+判定，手冊 §6.3）+ 工程模式「重複性測試」選單（跑當前影像 N=30）；另完成**真直度真值**（見下決策列）。純 Domain 全測（測試 40→49 套）。⏸ **剩：完整 GR&R**（操作員/零件變異需真實重複取像＝硬體） |
...
| 真直度真值（peak-to-peak 垂直帶）→ v2 | v1 用 ResidualRms 近似（UI/報表標「approx」）；升級＝各邊點到擬合線垂距 max−min 帶寬 | 2026-06-27 | **Done 2026-07-23**（`Domain/Gdt/StraightnessBand.PeakToPeakPx`，MeasureLine 存 `ToolRunResult.StraightnessBandPx`、GD&T straightness 改讀；純幾何全測；「approx」標記已改「真值」） |
...
| 操作員/工程師模式的密碼保護與操作紀錄（稽核） | 分流本身先驗證是否正確；v1 用無密碼選單開關即可，密碼可日後以極小成本疊加。目前無稽核/合規要求。計畫書：`docs/superpowers/plans/2026-07-20-operator-engineer-ui-split.md` | 2026-07-20 | Pending |
